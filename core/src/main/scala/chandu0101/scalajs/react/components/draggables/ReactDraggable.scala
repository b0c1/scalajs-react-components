...
object ReactDraggable {
  ...
  val component = ReactComponentB[Props]("ReactDraggable").initialStateP(
    p => State(
      clientX = p.start.x.toInt, clientY = p.start.y.toInt
    )
      ...
      (P, C, S, B) => {
        ...
        <.div(^.classSet1("react-draggable", "react-draggable-dragging" -> S.dragging), stl)(
          ^.onMouseDown ==> B.handleDragStart, ^.onMouseUp ==> B.handleDragEnd
        )(
            C
          )
      ...
      (scope, nextProps) => {
        if (nextProps.moveOnStartChange) scope.modState(
          _.copy(
            clientX = nextProps.start.x.toInt, clientY = nextProps.start.y.toInt
          )
        )
      }
  ...
  case class Props(cancel: String,
                   onDrag: EventRElementPositionAny,
                   ...
                   zIndex: Int,
                   axis: String,
                   onStop: EventRElementPositionAny,
                   start: RPoint,
                   onStart: EventRElementPositionAny,
                   onMouseDown: EventUnit,
                   handle: String,
                   minConstraints: RGrid,
                   maxConstraints: RGrid)
   ...
   * @return
   */
  def apply(cancel: String = "",
            onDrag: EventRElementPositionAny = null,
            ...
            zIndex: Int = 0,
            axis: String = "both",
            onStop: EventRElementPositionAny = null,
            start: RPoint = RPoint(0, 0),
            onStart: EventRElementPositionAny = null,
            onMouseDown: EventUnit = null,
            handle: String = "",
            minConstraints: RGrid = null,
            maxConstraints: RGrid = null)(children: ReactNode) =
