...
object ReactListView {
  ...
  val component = ReactComponentB[Props]("ReactListView").initialState(State()).backend(new Backend(_)).render(
      (P, S, B) => {
        val fItems = P.items.filter(item => item.toString.toLowerCase.contains(S.filterText.toLowerCase))
        <.div(P.style.reactListView)(
          P.showSearchBox ?= ReactSearchBox(onTextChange = B.onTextChange), <.ul(P.style.listGroup)(
            fItems.zipWithIndex.map {
              case (item, index) => {
                ...
                <.li(
                  P.style.listGroupItem,
                  selected ?= P.style.selectedListGroupItem,
                  first ?= P.style.firstListGroupItem,
                  last ?= P.style.lastListGroupItem,
                  hover ?= P.style.listGroupItemHover
                )(
                    ^.onClick --> B.onItemSelect(item.toString),
                    onMouseEnter --> B.onMouseEnter(index),
                    ^.onMouseOut --> B.onMouseLeave(index)
                  )(item)
              }
            }
          )
  ...
  trait Style {
    ...
    def selectedListGroupItem: TagMod = Seq(
      ^.color := "white", ^.fontWeight := 500, ^.backgroundColor := "#146699"
    )
    ...
    def listGroupItemHover: TagMod = Seq(
      ^.color := "#555555", ^.backgroundColor := "#ecf0f1"
    )
  ...
  def apply(items: List[String],
            onItemSelect: StringUnit = null,
            showSearchBox: Boolean = false,
            style: Style = new Style {},
            ref: js.UndefOr[String] = "",
            key: js.Any = {}) = component.set(key, ref)(Props(items, onItemSelect, showSearchBox, style))
