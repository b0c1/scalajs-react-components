...
object GoogleMap {
  ...
  val component = ReactComponentB[Props]("googleMap").initialState(State(loaded = false)).backend(new Backend(_)).render(
      (P, S, B) => {
   ...
   * @return
   */
  def apply(width: Int = 500,
            height: Int = 500,
            center: LatLng,
            zoom: Int = 4,
            markers: List[Marker] = Nil,
            url: String = "https://maps.googleapis.com/maps/api/js") = component(
    Props(
      width,
      height,
      center,
