...
object MuiAppBar {
  ...
  val component = ReactComponentB[Props]("MuiAppBar").stateless.backend(new Backend(_)).render(
      (P, C, S, B) => {
        MuiPaper(rounded = false, clsNames = cssMap1M(mui_app_bar, P.clsNames), zDepth = P.zDepth)(
          if (P.showMenuIconButton) MuiIconButton.withChildren(
            clsNames = Map(mui_app_bar_navigation_icon_button -> true),
            onTouchTap = B.onMenuIconButtonTouchTap,
            key = "iconButton"
          )(MuiNavigationMenuIcon())
          else "", if (P.title.nonEmpty) <.h1(^.cls := mui_app_bar_title, ^.key := P.title, P.title) else "", C
        )
      }
    ).build

  case class Props(clsNames: CssClassType,
                   onMenuIconButtonTouchTap: REventIUnit,
  ...
  def apply(clsNames: CssClassType = Map(),
            onMenuIconButtonTouchTap: REventIUnit = null,
            ref: js.UndefOr[String] = "",
            zDepth: Int = 0,
            key: js.Any = {},
