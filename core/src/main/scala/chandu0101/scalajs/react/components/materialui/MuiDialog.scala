...
object MuiDialog {
  ...
  lazy val component = ReactComponentB[Props]("MuiDialog").initialStateP(p => State(p.openImmediately)).backend(
      new Backend(
        _
      )
    ).render(
      (P, C, S, B) => {
        MuiDialogWindow(ref = theDialogWindowRef, clsNames = Map(mui_dialog -> true), actions = B.getDialogActions)(
          <.h3(^.cls := mui_dialog_title, ^.key := "title", P.title), <.div(
            ^.ref := theDialogContentRef,
            ^.key := "content",
            ^.cls := mui_dialog_content
          )(
            C
          )
        )
  ...
  case class Props(onDismiss: REventIUnit,
                   onShow: REventIUnit,
  ...
  def apply(onDismiss: REventIUnit = null,
            onShow: REventIUnit = null,
