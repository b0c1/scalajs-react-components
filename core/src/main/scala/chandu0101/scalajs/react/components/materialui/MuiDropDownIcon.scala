...
object MuiDropDownIcon {
  ...
  val component = ReactComponentB[Props]("MuiDropDownIcon").initialState(State(open = false)).backend(new Backend(_)).render(
      (P, C, S, B) => {
        val classes = CommonUtils.cssMap1M(mui_drop_down_icon, P.clsNames, mui_open -> S.open)
        <.div(^.classSetM(classes))(
          <.div(
            ^.cls := mui_menu_control, ^.onClick ==> B.onControlClick, P.iconClassName.nonEmpty ?= MuiFontIcon(
              className = P.iconClassName, C
            )
          ), MuiMenu(menuItems = P.menuItems, hideable = true, visible = S.open, onItemClick = B.onMenuItemClick)
        )
      }
    ).configure(ClickAwayable.mixin).build

  case class Props(iconClassName: String,
                   clsNames: CssClassType,
                   onChange: REventIIntStringUnit,
                   menuItems: List[MuiMenu.Item])

  // useful  when you want use svg icon instead of font-icon
  def withChildren(iconClassName: String = "",
                   clsNames: CssClassType = Map(),
                   onChange: REventIIntStringUnit = null,
                   menuItems: List[MuiMenu.Item],
                   key: js.Any = {},
                   ref: js.UndefOr[String] = "")(children: ReactNode*) = {
