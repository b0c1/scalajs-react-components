...
object MuiDropdownMenu {
  ...
  val component = ReactComponentB[Props]("MuiDropdownMenu").initialStateP(p => State(selectedIndex = p.selectedIndex)).backend(
      new Backend(_)
    ).render(
      (P, S, B) => {
    ...
    ).domType[html.Element].configure(ClickAwayable.mixin).componentDidMount(
      $ => {
