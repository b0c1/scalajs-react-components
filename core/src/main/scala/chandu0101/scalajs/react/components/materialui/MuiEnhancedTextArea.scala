...
object MuiEnhancedTextArea {
  ...
  val component = ReactComponentB[Props]("MuiEnhancedTextArea").initialStateP(p => State(p.rows * 24)).backend(
      new Backend(
        _
      )
    ).render(
      (P, S, B) => {
        ...
        <.div(classes)(
          <.textarea(
            ^.ref := theShadowRef,
            ^.cls := "mui-enhanced-textarea-shadow",
            ^.rows := P.rows,
            ^.tabIndex := "-1"
          ), <.textarea(
            ^.ref := theInputRef,
  ...
  def apply(onBlur: REventIUnit = null,
            ...
            onHeightChange: REventIIntUnit = null,
