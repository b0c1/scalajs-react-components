...
object MuiFlatButton {
  ...
  val component = ReactComponentB[Props]("MuiFlatButton").stateless.backend(new Backend(_)).render(
      (P, C, S, B) => {
        val classes = CommonUtils.cssMap1M(
          mui_flat_button, P.clsNames, mui_is_primary -> P.primary, mui_is_secondary -> (!P.primary && P.secondary)
        )
        MuiEnhancedButton(
          clsNames = classes,
          disabled = P.disabled,
  ...
  case class Props(disableFocusRipple: Boolean,
  ...
  def withChildren(disableFocusRipple: Boolean = true,
  ...
  def apply(disableFocusRipple: Boolean = true,
            disableTouchRipple: Boolean = false,
            clsNames: CssClassType = Map(),
            label: String,
            onTouchTap: REventIUnit = null,
