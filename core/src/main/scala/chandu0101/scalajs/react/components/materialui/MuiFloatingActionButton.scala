...
object MuiFloatingActionButton {
  ...
  val component = ReactComponentB[Props]("MuiFloatingActionButton").initialStateP(
      p => {
      ...
      (P, C, S, B) => {
        val classes = CommonUtils.cssMap1M(
          mui_floating_action_button, P.clsNames, mui_is_mini -> P.mini, mui_is_secondary -> P.secondary
        )
        MuiPaper(clsNames = classes, zDepth = S.zDepth, circle = true)(
          MuiEnhancedButton(
            clsNames = Map(mui_floating_action_button_container -> true),
