...
object MuiIconButton {
  ...
  val component     = ReactComponentB[Props]("MuiIconButton").initialState(State(tooltipShown = false)).backend(
      new Backend(
        _
      )
    ).render(
      (P, C, S, B) => {
        MuiEnhancedButton(
          clsNames = P.clsNames + (mui_icon_button -> true),
          ...
          onMouseOver = B.handleMouseOver
            ...
            if (P.tooltip.nonEmpty) {
              MuiTooltip(
                ref = theToolTipRef,
                clsNames = Map(mui_icon_button_tooltip -> true),
                label = P.tooltip,
                show = S.tooltipShown,
                touch = P.touch,
                key = "icontooltip"
              )
            } else "", MuiFontIcon(className = P.iconClassName), C
          )
      }
    ).componentDidMount(scope => if (scope.props.tooltip.nonEmpty) scope.backend.positionTooltip).build

  def withChildren(iconClassName: String = "",
                   onBlur: REventIUnit = null,
                   touch: Boolean = false,
                   url: String = "",
