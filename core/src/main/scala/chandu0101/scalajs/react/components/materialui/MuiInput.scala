...
object MuiInput {
  ...
  case class Props(defaultValue: String,
                   name: String,
                   multiline: Boolean,
                   step: Long,
                   description: String,
                   onChange: REventIStringAny,
                   min: Long,
                   inputStyle: String,
                   placeholder: String,
                   valueLink: Boolean,
  ...
  def getInputElement(props: Props, backend: Backend, state: State) = {
    ...
    if (props.multiline) {
      if (props.valueLink) <.textarea(
        ...
        ^.rows := state.rows,
        ^.value := state.value,
        ^.onChange ==> backend.onTextAreaChange,
        ^.required := props.required,
        ^.`type` := props.tpe,
        ^.name := props.name,
        ^.disabled := props.disabled
      ...
      if (props.valueLink) <.input(textClasses)(
        ^.ref := theInputRef,
        ^.placeholder := placeHolderText,
        ^.required := props.required,
        ^.`type` := props.tpe,
        ^.name := props.name,
        ^.disabled := props.disabled,
        ^.onFocus ==> backend.onInputFocus,
        ^.onClick ==> backend.onInputTouchTap
      )
      else <.input(textClasses)(
        ^.ref := theInputRef,
        ^.placeholder := placeHolderText,
        ^.value := state.value,
        ^.onChange ==> backend.onInputChange,
        ^.required := props.required,
        ^.`type` := props.tpe,
        ^.name := props.name,
        ^.disabled := props.disabled,
  ...
  val component = ReactComponentB[Props]("input").initialStateP(
      p => State(
        p.defaultValue,
        p.rows,
        p.error
      ...
      (P, S, B) => {
        val classes = CommonUtils.cssMapM(
          P.classNames,
          mui_input -> true,
        ...
        <.div(^.classSetM(classes))(
          ...
          if (P.inlinePlaceHolder) ""
          else <.span(
            ^.cls := mui_input_placeholder,
  ...
  def apply(defaultValue: String = "",
            ...
            step: Long = 0,
            description: String = "",
            onChange: REventIStringAny = null,
            min: Long = 0,
            inputStyle: String = "",
            placeholder: String = "",
            valueLink: Boolean = false,
            max: Long = 0,
            error: String = "",
            tpe: String = "text",
