...
object MuiLeftNav {
  ...
  val component = ReactComponentB[Props]("leftnav").initialStateP(p => State(p.docked)).backend(new Backend(_)).render(
      (P, S, B) => {
        val classes = CommonUtils.cssMap1M(
          mui_left_nav, P.classNames, mui_closed -> !S.open
        )
        <.div(^.classSetM(classes))(
          if (!P.docked) MuiOverlay(show = S.open, onTouchTap = B.onOverlayTouchTap _) else "", MuiPaper(
            clsNames = Map(
              mui_left_nav_menu -> true
