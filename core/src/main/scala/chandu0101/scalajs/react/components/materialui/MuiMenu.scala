...
object MuiMenu {
  ...
  case class Item(items: List[Item] = List(),
                  onToggle: REventIIntBooleanUnit = null,
                  number: String = "",
                  iconRight: String = "",
                  onClick: REventIIntStringUnit = null,
  ...
  val menuItem = ReactComponentB[(Item, Int, Int, Int, Backend)]("menuchildrens").render(
      P => {
        ...
        item.mtype match {
          ...
          case Types.NESTED => MuiNestedMenuItem(
            index = item.index,
            text = item.text,
            menuItems = item.items,
            zDepth = zDepth,
            onItemClick = b.onNestedItemClick,
            key = item.index
          )
          case _ => MuiMenuItem(
  ...
  lazy val component = ReactComponentB[Props]("MuiMenu").initialState(State(nestedMenuShown = false)).backend(
      new Backend(
        _
      )
    ).render(
      (P, S, B) => {
        MuiPaper(
          ref = thePaperMenuRef, clsNames = P.classNames.++(
            Map(
              mui_menu -> true,
              mui_menu_hideable -> P.hideable,
              mui_visible -> P.visible
            )
          ), zDepth = P.zDepth
        )(
    ...
    ).domType[html.Element].componentDidMount(
      scope => {
  ...
  case class Props(menuItems: List[MuiMenu.Item],
                   ...
                   autoWidth: Boolean,
                   hideable: Boolean,
                   onToggleClick: REventIUnit,
                   selectedIndex: Int,
                   onItemClick: REventIIntStringUnit,
                   classNames: CssClassType,
                   onItemToggle: REventIIntBooleanUnit)
