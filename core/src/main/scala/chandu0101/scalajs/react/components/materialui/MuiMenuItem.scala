...
object MuiMenuItem {
  ...
  val component = ReactComponentB[Props]("MuiMenuItem").stateless.backend(new Backend(_)).render(
      (P, C, S, B) => {
        val classes = CommonUtils.cssMap1M(
          mui_menu_item, P.clsNames, mui_is_selected -> P.selected
        )
        <.div(
          ^.classSetM(classes),
          ^.key := P.index,
          ^.onClick ==> B.onClick,
          P.iconClassName.nonEmpty ?= MuiFontIcon(
            className = s"mui-menu-item-icon ${P.iconClassName }"
          ),
          C,
  ...
  case class Props(onToggle: REventIIntBooleanUnit,
                   iconClassName: String,
                   number: String,
                   onClick: REventIIntStringUnit,
                   toggle: Boolean,
                   data: String,
  ...
  def apply(onToggle: REventIIntBooleanUnit = null,
            iconClassName: String = "",
            number: String = "",
