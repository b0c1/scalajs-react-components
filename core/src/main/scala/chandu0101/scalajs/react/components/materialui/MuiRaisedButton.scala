...
object MuiRaisedButton {
  ...
  val component = ReactComponentB[Props]("MuiRaisedButton").initialStateP(
      p => {
      ...
      (P, C, S, B) => {
        val classes = CommonUtils.cssMap1M(
          mui_raised_button, P.clsNames, mui_is_primary -> P.primary, mui_is_secondary -> (!P.primary && P.secondary)
        )
        MuiPaper(clsNames = classes, zDepth = S.zDepth)(
          MuiEnhancedButton(
            clsNames = Map(mui_raised_button_container -> true),
  ...
  case class Props(onTouchStart: REventIUnit,
  ...
  def withChildrenapply(onTouchStart: REventIUnit = null,
                        ...
                        key: js.Any = {},
                        disableFocusRipple: Boolean = true,
  ...
  def apply(onTouchStart: REventIUnit = null,
            disableTouchRipple: Boolean = false,
            url: String = "",
            clsNames: CssClassType = Map(),
            label: String = "",
