...
object MuiSlider {
  ...
  val component = ReactComponentB[Props]("radioButton").initialStateP(
      p => State(
        p.value,
        p.value / p.max
      )
      ...
      (P, S, B) => {
        ...
        val sliderClasses = cssMapM(
          P.clsNames,
          (mui_slider, true),
          (mui_slider_zero, S.percent == 0),
  ...
  def apply(clsNames: CssClassType = Map(),
            ...
            step: Int = 0,
            description: String = "",
            onChange: REventIDoubleUnit = null,
            min: Int = 0,
            max: Int = 1,
            error: String = "",
