...
object MuiTextField {
  ...
  val component        = ReactComponentB[Props]("MuiTextField").initialStateP(
      p => State(
        p.errorText,
        p.value != null || p.defaultValue != null
      )
      ...
      (P, S, B) => {
        ...
        val floatingTextElement = P.floatingLabelText.nonEmpty ?= <.label(
          ^.cls := "mui-text-field-floating-label",
          ^.htmlFor := inputId
        )(P.floatingLabelText)
        val inputElement: TagMod = if (P.multiLine)
                                     MuiEnhancedTextArea(
                                       ref = theMultiInputRef,
      ...
      (scope, nextProps) => {
        ...
        else if (nextProps.defaultValue != null) scope.modState(
          _.copy(
