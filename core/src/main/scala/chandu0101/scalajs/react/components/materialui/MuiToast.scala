...
object MuiToast {
  ...
  val component = ReactComponentB[Props]("radioButton").initialState(State(open = false)).backend(new Backend(_)).render(
      (P, S, B) => {
        val classes = CommonUtils.cssMapM(
          P.classNames, mui_toast -> true, mui_open -> S.open
        )
  ...
  def apply(onClick: REventIUnit = null,
            icon: String = "",
            open: Boolean = false,
            message: String = "",
  ...
  case class Props(onClick: REventIUnit,
                   icon: String,
                   open: Boolean,
                   message: String,
