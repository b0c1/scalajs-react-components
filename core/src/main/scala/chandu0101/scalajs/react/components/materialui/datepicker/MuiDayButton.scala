...
object MuiDayButton {
  ...
  val component = ReactComponentB[Props]("MuiDayButton").stateless.backend(new Backend(_)).render(
      (P, S, B) => {
        val classes = CommonUtils.cssMap(
          mui_date_picker_day_button,
  ...
  def apply(clsNames: CssClassType = Map(),
            ...
            key: js.Any = {},
            date: Date = null,
            onTouchTap: REventIDateUnit = null) = component.set(key, ref)(
    Props(
      clsNames,
      ref,
      selected,
