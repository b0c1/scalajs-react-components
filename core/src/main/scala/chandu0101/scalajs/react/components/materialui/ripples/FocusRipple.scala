...
object FocusRipple {
  ...
  val component = ReactComponentB[Props]("FocusRipple").stateless.backend(new Backend(_)).render(
      (P, S, B) => {
        val classes = CommonUtils.cssMap1M(
          mui_focus_ripple, P.clsNames, mui_is_shown -> P.show
        )
        <.div(^.classSetM(classes))(
          <.div(^.cls := mui_focus_ripple_inner)
        )
      }
    ).domType[html.Element].componentDidMount(scope => scope.backend.setRippleSize(scope.getDOMNode())).build
