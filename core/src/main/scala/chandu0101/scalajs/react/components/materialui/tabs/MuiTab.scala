...
object MuiTab {
  ...
  val component = ReactComponentB[Props]("MuiTab").initialState(State(false)).backend(new Backend(_)).render(
      (P, S, B) => {
  ...
  def apply(width: Int = 0,
            ...
            label: String,
            route: String,
            handleTouchTap: StringIntUnit = null,
