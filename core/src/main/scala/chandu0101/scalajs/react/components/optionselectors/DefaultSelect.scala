...
object DefaultSelect {
  ...
  val component = ReactComponentB[Props]("DefaultSelect").stateless.backend(new Backend(_)).render(
      (P, S, B) => {
        val reactselect: String = "reactselect"
        <.div(^.cls := reactselect)(
          <.label(^.htmlFor := reactselect)(<.strong(P.label)), <.select(
            ^.paddingLeft := "5px",
            ^.id := reactselect,
            ^.value := P.value,
            ^.onChange ==> B.onChange
  ...
  def apply(ref: js.UndefOr[String] = "",
            ...
            options: List[String],
            value: String,
            onChange: StringUnit) = component.set(key, ref)(Props(label, options, value, onChange))
}
