...
object Pager {
  ...
  val component = ReactComponentB[Props]("Pager").stateless.backend(new Backend(_)).render(
      (P, S, B) => {
        //    val prevClasses = Map(
        //                           P.css.pager -> (P.offset == 0) )
        //    val nextClasses = Map(P.css.pager -> (P.offset + P.itemsPerPage > P.totalItems))
        val prevClasses = Map(
          "previous" -> (P.offset > 0), "disabled" -> (P.offset == 0)
        )
        val nextClasses = Map(
          "next" -> (P.offset + P.itemsPerPage < P.totalItems),
  ...
  case class Props(itemsPerPage: Int,
                   totalItems: Int,
                   offset: Int,
                   nextClick: SyntheticEvent[html.Anchor] => Unit,
                   previousClick: SyntheticEvent[html.Anchor] => Unit,
                   css: CSS)
