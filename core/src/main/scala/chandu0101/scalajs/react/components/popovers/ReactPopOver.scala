...
object ReactPopOver {
  ...
  trait Style {
    ...
    def popover: TagMod = Seq(
      ...
      ^.whiteSpace := "normal"
    ).++(backgroundClipPreFixer("padding-box")).++(boxShadowPreFixer("0 5px 10px rgba(0, 0, 0, 0.2)"))
    ...
    def popoverTopArrowAfter = Seq(
      ^.marginLeft := "-10px", ^.borderBottomWidth := 0, ^.borderTopColor := "#ffffff", ^.bottom := "1px"
    )
    ...
    def popoverRightArrowAfter = Seq(
      ^.left := "1px", ^.borderLeftWidth := 0, ^.borderRightColor := "#ffffff", ^.bottom := "-10px"
    )
    ...
    def popoverBottomArrowAfter = Seq(
      ^.top := "1px", ^.marginLeft := "-10px", ^.borderTopWidth := 0, ^.borderBottomColor := "#ffffff"
    )
    ...
    def popoverLeftArrowAfter = Seq(
      ^.right := "1px", ^.borderRightWidth := 0, ^.borderLeftColor := "#ffffff", ^.bottom := "-10px"
    )
  }
  ...
  val component = ReactComponentB[Props]("ReactPopover").initialState(State(open = false)).backend(new Backend(_)).render(
      (P, C, S, B) => {
        <.div(
          P.style.popover,
          ...
          S.open ?= (^.left := S.left)
            ...
            <.div(
              ...
              B.arrowAfter
            ), !P.title.isEmpty ?= <.h3(P.style.popoverTitle)(P.title), <.div(P.style.popoverContent)(
          C
        )
          )
      }
  ...
  def apply(title: String = "",
            ...
            key: js.Any = {},
            style: Style = new Style {})(children: ReactNode*) = component.set(key, ref)(
