...
object ReactSearchBox {
  ...
  trait Style {
    def searchBox: TagMod = Seq(
      ^.position := "relative", ^.marginBottom := "10px"
    )

    def searchInput: TagMod = Seq(
      ^.display := "block",
      ^.border := "none",
      ^.fontSize := "13px",
      ^.fontWeight := 300,
    ...
    def searchInputFocus: TagMod = Seq(
      ^.outline := "none", ^.boxShadow := "none", ^.borderBottom := "1.5px solid #03a9f4"
    )
  ...
  val component = ReactComponentB[Props]("ReactSearchBox").initialState(State()).backend(new Backend(_)).render(
      (P, S, B) => {
        <.div(P.style.searchBox)(
          <.input(
            P.style.searchInput,
            S.focused ?= P.style.searchInputFocus,
            ^.placeholder := "Search ..",
  ...
  def apply(onTextChange: StringUnit,
