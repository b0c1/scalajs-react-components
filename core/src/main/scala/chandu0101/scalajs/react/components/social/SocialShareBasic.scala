...
object SocialShareBasic {
  ...
  class Backend(t: BackendScope[Props, _]) {
    ...
    def tweetMe(message: Tweet) = {
      ...
      val url = parameterizeUrl(
        host, Map(
          "url" -> message.url, "text" -> message.text, "via" -> message.via, "hashtags" -> message.hashTags.mkString(
            ","
          )
        )
      )
    ...
    def openUrl(url: String) = dom.window.open(
      url = url,
      target = "_blank",
      features = "width=600,height=400,left=100,top=100"
    )
    ...
    def tumblrLink(message: TumblrLink) = {
      ...
      val url = parameterizeUrl(
        host, Map(
          "url" -> message.url, "name" -> message.name, "description" -> message.description
        )
      )
      openUrl(url)
    }
    ...
    def pinIt(message: Pintrest) = {
      ...
      val url = parameterizeUrl(
        host, Map(
          "url" -> message.url, "media" -> message.media, "description" -> message.description
        )
      )
      openUrl(url)
    }
  ...
  val component = ReactComponentB[Props]("SocialShareBasic").stateless.backend(new Backend(_)).render(
      (P, S, B) => {
  ...
  case class Props(tweet: Tweet,
                   css: String,
                   fbPost: FaceBookPost,
                   gplusPost: GooglePlusPost,
  ...
  def apply(tweet: Tweet = null,
            ...
            key: js.Any = {}) = component.set(key, ref)(
    Props(
      tweet,
      css,
      fbPost,
