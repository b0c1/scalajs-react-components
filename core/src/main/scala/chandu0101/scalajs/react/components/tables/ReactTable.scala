...
object ReactTable {
  ...
  def getDateSort(key: String) = (m1: Model,
                                  m2: Model) => m1(key).asInstanceOf[Date].getTime() < m2(key).asInstanceOf[Date].getTime()
  ...
  val tableHeader = ReactComponentB[(Props, Backend, State)]("reactTableHeader").render(
      P => {
        val (props, b, state) = P
        <.thead(
          if (props.config.nonEmpty) {
            props.columns.map {
              item => {
                ...
                if (f != null) <.th(^.cls := b.sortClass(item), ^.cursor := "pointer", ^.onClick --> b.sort(f, item))(
                  item.capitalize
                )
  ...
  val tableRow = ReactComponentB[(Model, List[Config], List[String])]("SeedRow").render(
      P => {
        val (row, config, columns) = P
        <.tr(
          if (config.nonEmpty) {
            columns.map {
              item => val f = getRenderFunction(item, config); if (f != null) <.td(f(row(item)))
                                                               else <.td(
                                                                 row(item).toString
                                                               )
            }
  ...
  val tableRows = ReactComponentB[(State, Props)]("tableRows").render(
      P => {
        val (state, props) = P
        val rows = state.filteredModels.slice(
          state.offset,
          state.offset + state.rowsPerPage
        ).zipWithIndex.map { case (row, i) => tableRow.withKey(i)((row, props.config, props.columns)) }
        <.tbody(
          rows
        )
      }
      ...
      P => {
        ...
        <.div(^.cls := props.css.tableResponsive)(
          <.table(^.cls := props.css.table)(
            tableHeader((props, b, state)), tableRows((state, props))
          )
        )
      }
    ).build

  val searchBar = ReactComponentB[(Props, (ReactEventI) => Unit)]("searchBar").render(
      P => {
        val (props, fn) = P
  ...
  val settingsBar = ReactComponentB[(Props, Backend, State)]("settingbar").render(
      P => {
        val (p, b, s) = P
        var value = ""
        ...
        if (total > p.rowsPerPage) {
          ...
          options = immutable.Range.inclusive(
            p.rowsPerPage,
            total,
            10 * (total / 100 + 1)
          ).:+(total).toList.map(_.toString)
        }
        <.div(^.cls := "settings")(
  ...
  val component = ReactComponentB[Props]("ReactTable").initialStateP(
      p => State(
        filterText = "",
        offset = 0,
        p.rowsPerPage,
      ...
      (P, S, B) => {
        <.div(^.cls := "reacttable")(
          searchBar((P, B.onTextChange)), settingsBar((P, B, S)), tableC((P, S, B)), Pager(
            S.rowsPerPage,
  ...
  case class CSS(table: String = "table table-hover",
                 tableResponsive: String = "table-responsive-vertical shadow-z-1",
                 ascendingIcon: String = "ascendingIcon",
                 descendingIcon: String = "descendingIcon",
                 searchBoxForm: String = "form-group",
                 searchBoxControl: String = "form-control")
