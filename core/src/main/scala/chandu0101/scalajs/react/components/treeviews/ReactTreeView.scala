...
object ReactTreeView {
  ...
  trait Style {
    ...
    def selectedTreeItemContent = Seq(
      ^.backgroundColor := "#1B8EB0", ^.color := "white", ^.fontWeight := 400, ^.padding := "0 7px"
    )
  ...
  case class State(filterText: String = "",
                   ...
                   selectedNode: BackendScope[NodeProps, NodeState] = null)
  ...
  class NodeBackend(t: BackendScope[NodeProps, NodeState]) {
    ...
    def isFilterTextExist(filterText: String, data: TreeItem): Boolean = {
      def loop(data: Seq[TreeItem]): Boolean = {
        data.view.exists(
          item => if (item.children.isEmpty) item.item.toString.toLowerCase.contains(filterText.toLowerCase)
                  else loop(
                    item.children
                  )
        )
  ...
  case class NodeProps(root: TreeItem,
                       ...
                       depth: Int = 0,
                       parent: String = "",
                       onNodeSelect: (BackendScope[NodeProps, NodeState]) => Any,
                       filterText: String,
                       style: Style,
                       filterMode: Boolean)

  lazy val TreeNode: ReactComponentC.ReqProps[NodeProps, NodeState, NodeBackend, TopNode] = ReactComponentB[NodeProps](
    "ReactTreeNode"
  ).initialStateP(p => if (p.open) NodeState(p.root.children) else NodeState()).backend(new NodeBackend(_)).render(
      (P, S, B) => {
        val children = S.children
        val depth = P.depth + 1
        val parent = if (P.parent.isEmpty) P.root.item.toString else s"${P.parent }<-${P.root.item.toString }"
        val treeMenuToggle: TagMod = if (children.nonEmpty) <.span(
          ^.onClick ==> B.onTreeMenuToggle,
          ^.key := "arrow",
          P.style.treeItemBefore
        )("▼")
                                     else if (P.root.children.nonEmpty && children.isEmpty) <.span(
          ^.onClick ==> B.onTreeMenuToggle,
          ^.key := "arrow",
          P.style.treeItemBefore
        )("▶")
        ...
        )(<.span(S.selected ?= P.style.selectedTreeItemContent, ^.onClick ==> B.onItemSelect, P.root.item.toString))(
          <.ul(P.style.treeGroup)(
            children.map(
              child => {
                val key: js.Any = s"$parent$depth${child.item }"
                val open = if (!P.filterText.trim.isEmpty) true else false
                B.isFilterTextExist(P.filterText, child) ?= TreeNode.withKey(key)(
                  P.copy(
                    root = child,
                    depth = depth,
                    parent = parent,
  ...
  val component = ReactComponentB[Props]("ReactTreeView").initialState(State()).backend(new Backend(_)).render(
      (P, S, B) => {
  ...
  case class Props(root: TreeItem,
                   open: Boolean,
                   onItemSelect: StringStringIntAny,
                   showSearchBox: Boolean,
                   style: Style)

  def apply(root: TreeItem,
            openByDefault: Boolean = false,
            onItemSelect: StringStringIntAny = null,
            showSearchBox: Boolean = false,
            ref: js.UndefOr[String] = "",
            key: js.Any = "dude",
            style: Style = new Style {}) = component.set(key, ref)(
    Props(
      root,
      openByDefault,
      onItemSelect,
