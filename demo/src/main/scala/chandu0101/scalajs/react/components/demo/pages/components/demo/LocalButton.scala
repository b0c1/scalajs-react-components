...
object LocalDemoButton {
  ...
  trait Style {
    ...
    val buttonHover: TagMod = Seq(
      ^.backgroundColor := "#DA423E", ^.textDecoration := "none"
    )
  ...
  val component = ReactComponentB[Props]("LocalDemoButton").initialState(State()).backend(new Backend(_)).render(
      (P, S, B) => {
  ...
  def apply(name: String,
            onButtonClick: REventHUnit = null,
            linkButton: Boolean = false,
            href: String = "",
            style: Style = new Style {},
            ref: js.UndefOr[String] = "",
            key: js.Any = {}) = component.set(key, ref)(Props(name, onButtonClick, linkButton, href, style))
