...
object MToast {
  ...
  val component = ReactComponentB[Unit]("plain").stateless.backend(new Backend(_)).render(
      (P, S, B) => {
        <.div(
          <.h2(^.cls := "mui-font-style-headline")("MuiToast"), CodeExample(code)(
            MuiRaisedButton(label = "MuiToast Demo", onTouchTap = B.onButtonTap), MuiToast(
              ref = theToastRef,
              message = "Hola I am MuiToast"
            )
          )
        )
      }
