...
object MuiLeftNavDemo {
  ...
  val menuItems = List(
    MuiMenu.Item(text = "Material ui", route = "mui"), MuiMenu.Item(text = "React table", route = "rct"), MuiMenu.Item(
      mtype = MuiMenu.Types.SUBHEADER,
      text = "Resources"
    ), MuiMenu.Item(
      mtype = MuiMenu.Types.LINK,
      text = "Github",
      route = "https://github.com/chandu0101/scalajs-react-components"
    )
  )
  ...
  val component = ReactComponentB[Unit]("MuiLeftNavDemo").initialState(State(docked = false)).backend(new Backend(_)).render(
      (P, S, B) => {
        <.div(
          <.h3("Left Nav"), CodeExample(code)(
            <.div(
