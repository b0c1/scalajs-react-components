...
object MuiTextFieldDemo {
  ...
  val component = ReactComponentB[Unit]("MuiTextFieldDemo").initialState(State()).backend(new Backend(_)).render(
      (P, S, B) => {
        <.div(
          <.h3("Text Field", ^.key := "heading"), CodeExample(code, key = "codeexample")(
            <.div(Style.textFieldExamples, ^.key := "examples")(
              <.div(Style.textFieldGroup, ^.key := "group")(
                MuiTextField(hintText = "Hint Text", defaultValue = "Default Value", key = "inline2"), MuiTextField(
                  hintText = "Hint Text",
                  value = S.propValue,
                  onChange = B.handleInputChange,
                  key = "inline3"
                ), MuiTextField(hintText = "Hint Text(multi line )", multiLine = true, key = "inline4"), MuiTextField(
                  hintText = "Hint Text",
                  errorText = "fix me now",
                  key = "inline5"
                ), MuiTextField(hintText = " Disabled Hint Text", disabled = true, key = "inline6"), MuiTextField(
