...
object CustomCellReactTable {
  ...
  val component = ReactComponentB[Unit]("plain").render(
      P => {
        <.div(
          <.h2(^.cls := "mui-font-style-headline")("Custom Cell Factory"), CodeExample(code)(
            ReactTable(data = data, columns = columns, config = config)
          )
        )
      }
  ...
  def customFname = (fname: Any) => {
    val name = fname.toString; if (name.startsWith("J")) <.span(^.backgroundColor := "grey")(
      name
    ).render
                               else <.span(name).render
  }
