...
object GithubUser {
  ...
  val component = ReactComponentB[Props]("GithubUser").initialState(State(false)).backend(new Backend(_)).render(
      (P, S, B) => {
        <.a(Styles.userGroup, ^.href := P.user.html_url)(
          <.img(Styles.userIcon, ^.src := P.user.avatar_url), <.span(Styles.userName)(P.user.login)
        )
      }
    ).build
