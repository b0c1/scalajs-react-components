...
object LeftnavPage {
  ...
  object Style {
    ...
    val pageBody = Seq(
      ^.borderLeft := "solid 1px #e0e0e0", ^.minHeight := "800px", ^.flex := 1, ^.padding := "20px"
    )

    val pageBodyContent = Seq(
      ^.borderRadius := "2px", ^.boxShadow := "0 1px 4px rgba(39, 54, 54, 0.79)", ^.maxWidth := "1024px"
    )
  ...
  val component = ReactComponentB[Props]("LeftnavPage").initialState(State(false)).backend(new Backend(_)).render(
      (P, S, B) => {
  ...
  def apply(menu: List[DemoLeftNav.Menu],
            content: ReactNode,
            ref: js.UndefOr[String] = "",
            key: js.Any = {}) = component.set(key, ref)(Props(menu, content))
