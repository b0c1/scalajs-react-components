...
object AppRouter {
  ...
  object AppPage extends RoutingRules {
    ...
    val socialShareIconsDemo: Loc = register(
      location(
        "#socialshare/demo",
        SocialShareIconsPage(SocialShareIconsDemo())
      )
    )
    ...
    override protected def interceptRender(i: InterceptionR): ReactElement = {
      <.div(
        AppHeader(), i.element, <.div(^.textAlign := "center", ^.key := "footer")(
          <.hr(), <.p("Built using scalajs-react")
        )
      )
    }
